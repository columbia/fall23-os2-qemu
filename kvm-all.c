...
int kvm_resume_inc_exe(void)
{
    if (kvm_vm_ioctl(kvm_state, KVM_ARM_RESUME_INC_EXE, 0) < 0)
	exit(-1);
    return 0;
}

int kvm_is_saving_vm(void)
{
    return kvm_state->migration_log;
}

int kvm_encrypt_buf(unsigned long uva, unsigned long out_uva)
{
    struct kvm_user_encrypt kue;
    kue.uva = uva;
    kue.out_uva = out_uva;
    if (kvm_vm_ioctl(kvm_state, KVM_ARM_ENCRYPT_BUF, &kue) < 0)
	exit(-1);
    return 0;
}

int kvm_decrypt_buf(unsigned long uva)
{
    if (kvm_vm_ioctl(kvm_state, KVM_ARM_DECRYPT_BUF, uva) < 0)
	exit(-1);
    return 0;
}

int kvm_is_zero_page(unsigned long uva)
{
    int ret = kvm_vm_ioctl(kvm_state, KVM_ARM_IS_ZERO_PAGE, uva);
    return ret;
}
